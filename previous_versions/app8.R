...
# Maximun number of peaks selects the fistr tow, but we could choose the ones with the highest intensity
# Windiw is calcualted *2, could be better
...
server <- function(input, output, session) {
  ...
  output$HisPlot <- renderPlot({
    ...
    Name <- sub(" .*", "", identifier(InitDf()$Files[[SampNum]]))
    # Number of labels
