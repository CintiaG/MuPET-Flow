...
File <- "/home/cintia/Documents/Sorbonne/Project/13_peaks_and_final_ploidy/00_raw_data/test/out_data/BEM  CZ2022-09-02.0001.fcs"

Flow <- read.FCS(File, emptyValue = FALSE, alter.names = TRUE)
                ...
                navbarPage(
                  ...
                  tabPanel("Navbar 1",
                           sidebarPanel(
                             ...
                             numericInput(inputId = "Window", label = "Window", value = 20, min = 1, max = 500, step = 5),
                             numericInput(inputId = "Peaks", label = "Peaks", value = 2, min = 1, max = 5, step = 1),
...
server <- function(input, output) {
  Flow <- reactive({
    inFile <- input$file1
    if(!is.null(inFile)){
      #read.csv(inFile$datapath, header = input$header, stringsAsFactors = FALSE, row.names = FALSE)
      read.FCS(inFile$datapath, emptyValue = FALSE, alter.names = TRUE)
    }
  })
  ...
  output$contents <- renderTable({
    ...
    for(i in 1:length(input$file2[, 1])){
      #upload[[i]] <- read.csv(input$file2[[i, 'datapath']])
      upload[[i]] <- read.FCS(input$file2[[i, 'datapath']], emptyValue = FALSE, alter.names = TRUE)
    }
  ...
  #for (i in 1:length(input$file2[,1])){
  #  Ls[[i]] <- read.FCS(input$file2$datapath, emptyValue = FALSE, alter.names = TRUE)
  #}
  ...
  output$dropdown <- renderUI({
    ...
    selectInput("Channels", label = "Channels", choices = names(Flow()), selected = names(Flow())[20])
  })

  output$plot1 <- renderPlot({
    req(Flow())
    # Get name for ploting and table
    Name <- sub(" .*", "", identifier(Flow()))
    PeaksDf <- NULL
    # if there is a for should be
    # Get channels information
    ChanNum <- grep(input$Channels, names(Flow()))
    # Calculate histogram
    Hist <- hist(exprs(Flow()[,ChanNum]), breaks = 1:1000, plot = FALSE)
